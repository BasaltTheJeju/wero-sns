...
public class GetDiaryResponseDto extends ResponseDto {
    ...
    private GetDiaryResponseDto(DiaryEntity diaryEntity, List<DiaryImageEntity> imageEntities){//엔티티를 받는 생성자
        super(ResponseCode.SUCCESS, ResponseMessage.SUCCESS);//부모로부터 상속받은 프로퍼티들 채워주고

        List<String> diaryImageList = new ArrayList<>();

        for(DiaryImageEntity imageEntity : imageEntities){
            String diaryImage=imageEntity.getImage();
    ...
    public static ResponseEntity<GetDiaryResponseDto> success(DiaryEntity diaryEntity, List<DiaryImageEntity> imageEntities){//serviceImpl로 부터 diaryEntity를 받아서
        GetDiaryResponseDto result = new GetDiaryResponseDto(diaryEntity,imageEntities);//GetDiaryResponseDto를 생성한다. 다른 응답 dto들과는 다르게 엔티티를 넣어서 생성자 호출한다
